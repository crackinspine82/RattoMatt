...
/** Combined Structure page: syllabus tree (left) + full extract blocks (right). Uses structure item id; full extract saved via notes item. */
export default function CombinedStructureEditor() {
  ...
  const PREVIEW_WIDTH_KEY = 'curation-app-structure-preview-width';
  ...
  const UNDO_MAX = 5;
  type UndoSnapshot = { nodes: DraftNode[]; blocks: DraftNoteBlock[] };
  const [undoStack, setUndoStack] = useState<UndoSnapshot[]>([]);
  const nodesRef = useRef<DraftNode[]>([]);
  const blocksRef = useRef<DraftNoteBlock[]>([]);
  const contentAtFocusRef = useRef<{ blockId: string; content: string } | null>(null);
  const previousSectionTitleRef = useRef<{ nodeId: string; title: string } | null>(null);
  useEffect(() => {
  ...
  function pushUndo(snapshot?: UndoSnapshot) {
    const toPush: UndoSnapshot = snapshot ?? { nodes: nodesRef.current, blocks: blocksRef.current };
    setUndoStack((prev) => {
      const next = prev.length >= UNDO_MAX ? prev.slice(1) : prev;
      return [...next, JSON.parse(JSON.stringify(toPush))];
    });
  }
  ...
  useEffect(() => {
    if (!itemId) return;
    getFullExtract(itemId)
      .then((data) => {
        setNodes(data.nodes);
        setBlocks(data.blocks);
        setNotesItemId(data.notes_item_id);
        setUndoStack([]);
      })
  ...
  function handleSectionTitleSave(nodeId: string, newTitle: string) {
    ...
    const prev = previousSectionTitleRef.current;
    if (prev && prev.nodeId === nodeId) {
      if (prev.title !== titleToSave) {
        pushUndo({
          nodes: nodes.map((n) => (n.id === nodeId ? { ...n, title: prev.title } : n)),
          blocks: blocksRef.current,
        });
      }
      previousSectionTitleRef.current = null;
    }
    setNodes((prev) => prev.map((n) => (n.id === nodeId ? { ...n, title: titleToSave } : n)));
    setEditingNodeId(null);
  }

  function handleSectionTitleEditStart(nodeId: string, currentTitle: string) {
    previousSectionTitleRef.current = { nodeId, title: currentTitle };
  }

  function handleBlockFocus(blockId: string, contentHtml: string) {
    contentAtFocusRef.current = { blockId, content: contentHtml };
  }

  function handleBlockBlur(blockId: string) {
    const atFocus = contentAtFocusRef.current;
    if (!atFocus || atFocus.blockId !== blockId) return;
    const currentBlocks = blocksRef.current;
    const block = currentBlocks.find((b) => b.id === blockId);
    if (!block || block.content_html === atFocus.content) {
      contentAtFocusRef.current = null;
      return;
    }
    const snapshotBlocks = currentBlocks.map((b) =>
      b.id === blockId ? { ...b, content_html: atFocus.content } : b
    );
    pushUndo({ nodes: nodesRef.current, blocks: snapshotBlocks });
    contentAtFocusRef.current = null;
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden', color: 'var(--text)' }}>
      <header style={{ flexShrink: 0, padding: '16px 24px', borderBottom: '1px solid var(--border)', background: 'var(--header-bg)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 12, gap: 16, flexWrap: 'wrap' }}>
          <h1 style={{ margin: 0, fontSize: 22 }}>Structure (syllabus + full extract)</h1>
          <div style={{ display: 'flex', gap: 12 }}>
            <button type="button" onClick={handleUndo} disabled={undoStack.length === 0} title="Undo last action (up to 5)" style={{ padding: '10px 16px', fontSize: 14, fontWeight: 600, background: 'var(--surface)', color: 'var(--text)', border: '1px solid var(--border)', borderRadius: 6, cursor: undoStack.length === 0 ? 'not-allowed' : 'pointer' }}>
      ...
      {!notesItemId && !loading && <p style={{ padding: '0 24px', color: 'var(--text-muted)', fontSize: 14 }}>No full-extract notes item for this chapter. Run notes import for this chapter to edit content.</p>}
      {loading ? (
        ...
        >
          <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
            ...
            <div
              ref={contentRef}
              style={{ flex: 1, overflowY: 'auto', padding: 24, background: 'var(--bg)' }}
            >
              <p style={{ color: 'var(--text-muted)', marginBottom: 16 }}>{nodes.length} nodes, {blocks.length} full-extract blocks.</p>
                    ...
                    >
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                        {editingNodeId === n.id ? (
                          ...
                          <button
                            type="button"
                            onClick={() => { handleSectionTitleEditStart(n.id, n.title); setEditingNodeId(n.id); }}
                      ...
                      {nodeBlocks.length === 0 ? (
                        <p style={{ fontSize: 14, color: 'var(--text-muted)' }}>No content</p>
                      ) : (
                        <SortableContext items={nodeBlocks.map((b) => b.id)} strategy={verticalListSortingStrategy}>
                          {nodeBlocks.map((b) => (
                            <SortableBlockRow key={b.id} block={b}>
                              <div style={{ flex: 1, minWidth: 0 }}>
                                <RichTextBlockEditor block={b} onContentChange={handleBlockContentChange} itemId={itemId ?? undefined} onFocus={handleBlockFocus} onBlur={handleBlockBlur} />
                              </div>
                              <button
                                type="button"
            ...
            >
              ...
              <div className="structure-notes-preview">
                {flatOrder.map((n) => {
                  const nodeBlocks = [...(blocksByNode[n.id] ?? [])].sort(
                    (a, b) => a.sequence_number - b.sequence_number
                  );
                  return (
                    ...
                    >
                      ...
                      {nodeBlocks.length === 0 ? (
                        <p style={{ fontSize: 14, color: 'var(--text-muted)', margin: 0 }}>No content</p>
                      ) : (
                        nodeBlocks.map((b) => (
                          <div
                            key={b.id}
                            className="structure-notes-preview__block"
