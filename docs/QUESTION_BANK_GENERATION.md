...
**Output:** `out/sample_questions_Ch{N}_{discipline}.json` — same structure as existing sample_questions files for `curation:import`. Questions may include `scenario_data` (e.g. `image_placeholder_caption` for picture study, `image_instruction` for visual scenario) and, when run with `--from-db`, **section_ref** / **section_refs** (path strings) or for **structured_essay** **section_refs** as `[{ sub_part_key, path }, ...]` for per-sub-part resolution to syllabus nodes (see below).
...
- **With `--from-db`:** The 60% within-structure list comes from the API (the script resolves the chapter’s questions item id and calls the endpoint). You can use either:
  - **ADMIN_API_TOKEN** (recommended): calls `GET /admin/curation/items/:id/structure-images`. Set **ADMIN_API_KEY** in `backend/.env` and the same value as **ADMIN_API_TOKEN** where the script runs (e.g. same `.env`). No expiry; rotate by changing both.
  - **CURATION_API_TOKEN**: calls `GET /curation/items/:id/structure-images`. Set **CURATION_SCRIPT_API_KEY** in `backend/.env` and the same value as **CURATION_API_TOKEN** where the script runs; or use a token from SME login (expires in 7 days).
  Optionally set **CURATION_API_URL** (default `http://localhost:3000`). If the API call fails (network, 401, etc.), the run **aborts** with a clear error. The API returns image slug and published node ids; each (slug, node) becomes one question slot. In the curation app, slugs are set from the image **filename (no extension)** when uploading (multi-upload; duplicates skipped). No folder is used for 60% in this mode.
...
2. **Node-aware long-format (e.g. structured_essay)** — For types like `structured_essay`, the plan is built per **eligible node** (depth 0/1/2 with subtree word count ≥ `min_words` or descendant count ≥ `min_descendants`). When total essay count ≥ number of eligible nodes, **every eligible node gets at least one**; the remainder is distributed by depth weights (50% → depth 2, 30% → depth 1, 20% → depth 0) in round-robin within each depth. Otherwise, counts are distributed by depth with carry-forward. Batches are scoped to each node’s subtree so questions are generated for that section. For **structured_essay**, each question outputs **section_refs** as an array of three objects: `[{ sub_part_key: "i", path: "..." }, { sub_part_key: "ii", path: "..." }, { sub_part_key: "iii", path: "..." }]` (exact paths from the syllabus tree), so sub-parts (i), (ii), (iii) can map to different nodes for mastery. Other types use **section_ref** or **section_refs** (string array) as before.
...
6. **Resolution (merge-questions-node-ids)** — After generation, run the merge script so that **section_ref** / **section_refs** are resolved to **syllabus_node_id** (and for structured_essay, **sub_part_syllabus_node_ids**):
   ...
   - **structured_essay with section_refs (per sub-part):** When the question has **section_refs** as an array of `{ sub_part_key, path }` (sub_part_key `"i"`, `"ii"`, `"iii"`), each path is resolved to a node id. The question gets **sub_part_syllabus_node_ids** (array of `{ sub_part_key, syllabus_node_id }`) and **syllabus_node_id** is set to **null**. The flat list uses the first sub-part’s node for pairing when needed.
   - **Multiple refs (legacy):** For **section_ref** (string) or **section_refs** (array of strings), each ref is resolved to one node id; the question is assigned the **LCA** (lowest common ancestor) of those nodes.
   - **Unresolved:** If ref(s) cannot be resolved, **question.syllabus_node_id** is set to **null** (and no sub_part_syllabus_node_ids for that ref) and a warning is logged. There is no round-robin fallback for that question.
...
7. **Curation import** — When creating draft questions from the JSON, the importer uses **question.syllabus_node_id** when present; otherwise it uses **item.syllabus_node_id**. For **structured_essay** questions that have **sub_part_syllabus_node_ids** (array of `{ sub_part_key, syllabus_node_id }`), the importer sets **syllabus_node_id** on the draft question to **null** and inserts rows into **draft_question_sub_part_nodes** (one per sub_part_key i/ii/iii with a valid node id). Per-question and per-sub-part assignment from resolution is honoured.
